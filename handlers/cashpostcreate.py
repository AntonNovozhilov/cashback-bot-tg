...
from fsm.states import PostCachback
...
async def post_price_before(message: types.Message, state: FSMContext):
    ...
    if not message.text.isdigit():
        await message.answer(
            "Пожалуйста, введите цену числом без пробелов и символов (например, 1490):"
        )
        return
    if contains_emoji(message.text):
        await message.answer(
...
async def post_price_after(message: types.Message, state: FSMContext):
    ...
    if not message.text.isdigit():
        await message.answer(
            "Пожалуйста, введите цену числом без пробелов и символов (например, 1000):"
        )
        return
    ...
    if new_price >= old_price:
        await message.answer(
            "Стоимость после кешбека не может быть больше или равна стоимости до кешбека."
        )
        return
...
async def post_cashback(message: types.Message, state: FSMContext):
    ...
    if not message.text.isdigit():
        await message.answer(
            "Пожалуйста, введите кешбэк числом (например, 300 или 10):"
        )
        return
    cashback_value = int(message.text)
    cashback_type = "₽" if cashback_value > 100 else "%"
    await state.update_data(
        discount=cashback_value, cashback_type=cashback_type
    )
        ...
        inline_keyboard=[
            [
                types.InlineKeyboardButton(
                    ...
                    callback_data="send_to_mod_cash",
                )
            ],
            [
                types.InlineKeyboardButton(
                    text="🔄 Начать заново", callback_data="restart_post_cash"
                )
            ],
        ]
    )
