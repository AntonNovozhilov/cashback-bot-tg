...
from keyboards.inline import (cancel_keyboard, confirm_keyboard,
                              contact_keyboard, moderation_keyboard,
                              skip_photo_keyboard, type_keyboard)
from keyboards.inline_kb import finish_kb3
from models.models import ChatPrivatUser, Posts, User
...
async def start_post_creation(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(
        "Шаг 1/6. Что вы хотите разместить?", reply_markup=type_keyboard()
    )
...
async def process_post_type(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(
        f"Шаг 2/6. {question}", reply_markup=cancel_keyboard
    )
...
async def process_role(message: Message, state: FSMContext):
    ...
    await message.answer(
        "Шаг 3/6. Укажите контакт для связи", reply_markup=contact_keyboard()
    )
...
async def process_contact_self(callback: CallbackQuery, state: FSMContext):
    ...
    if not user.username:
        await callback.message.answer(
            "❗ У вас не установлен username. Введите контакт вручную:",
...
async def process_contact_custom(callback: CallbackQuery, state: FSMContext):
    await callback.message.answer(
        "Введите контакт вручную (например: @username):",
        reply_markup=cancel_keyboard,
    )
    await state.set_state(Post.contact_custom)
...
async def process_about_you(message: Message, state: FSMContext):
    ...
    await message.answer(
        "Шаг 4.2/6. Опишите свои услуги:", reply_markup=cancel_keyboard
    )
...
async def process_services(message: Message, state: FSMContext):
    ...
    await message.answer(
        "Шаг 4.3/6. Преимущества или кейсы:", reply_markup=cancel_keyboard
    )
...
async def process_about_company(message: Message, state: FSMContext):
    ...
    await message.answer(
        "Шаг 4.2/6. Опишите должность:", reply_markup=cancel_keyboard
    )
...
async def process_position(message: Message, state: FSMContext):
    ...
    await message.answer(
        "Шаг 4.3/6. Требования:", reply_markup=cancel_keyboard
    )
...
async def move_to_photo_step(message: Message, state: FSMContext):
    if message.text:
        key = (
            "cases"
            if await state.get_state() == Post.cases
            else "requirements"
        )
        await state.update_data({key: message.text})
    await message.answer(
        "Шаг 5/6. Прикрепите фото (или нажмите 'Пропустить')",
...
async def save_photo(message: Message, state: FSMContext):
    if message.media_group_id:
        media_group_buffer[message.media_group_id].append(
        ...
        if len(media_group_buffer[message.media_group_id]) == 1:
            ...
            await message.answer(
                "🖼 Фото добавлены. Нажмите кнопку ниже, чтобы завершить.",
                reply_markup=finish_kb3(),
            )
    else:
        data = await state.get_data()
...
async def confirm_send(callback: CallbackQuery, state: FSMContext):
    ...
    async with async_session() as session:
        ...
        result = await session.execute(
            select(ChatPrivatUser).where(
                ChatPrivatUser.user_id == callback.from_user.id
            )
        )
